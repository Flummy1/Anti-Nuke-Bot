...
import { minuteLimits, roles } from './LimitsMap'
...
export default class MinuteLimiter {
    private client: Core
    private repository: limiterRepository
    private actionName: keyof typeof minuteLimits
    private actions: Map<string, userActionData>

    constructor(core: Core, actionName: keyof typeof minuteLimits) {
        this.client = core
    ...
    public async applyAction(targetId: string, backupAction: () => Promise<void>) {
        ...
        const embed = new EmbedBuilder()
            .setTimestamp().setColor(`#2c58b1`)
            .setTitle(`Запись о действии | Минутный интервал`)
            .setFields([
                {
                    name: "Тип действия",
                    inline: true,
    ...
    private getLimit(target: GuildMember) {
        const limitTable = minuteLimits[this.actionName]
