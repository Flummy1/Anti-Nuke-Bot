...
export default class ChannelCreateListener implements IListener {
    ...
    constructor(client: Core) {
        ...
        this.minuteLimiter = new MinuteLimiter(client, `integrationCreate`)
        this.hourLimiter = new HourLimiter(client, `integrationCreate`)
    }
    ...
    public async execute(client: Core, logEntry: GuildAuditLogsEntry, guild: Guild): Promise<void> {
        ...
        if (logEntry.action !== AuditLogEvent.IntegrationCreate) return
