...
export default class ChannelCreateListener implements IListener {
    ...
    constructor(client: Core) {
        ...
        this.minuteLimiter = new MinuteLimiter(client, `integrationUpdate`)
        this.hourLimiter = new HourLimiter(client, `integrationUpdate`)
    }
    ...
    public async execute(client: Core, logEntry: GuildAuditLogsEntry, guild: Guild): Promise<void> {
        ...
        if (logEntry.action !== AuditLogEvent.IntegrationUpdate) return
